    ...
    <div className="rounded-2xl border border-sage-border/40 bg-white overflow-hidden shadow-card">
      ...
      <div className="px-5 pt-5 pb-4 border-b border-sage-border/30">
        <div className="flex items-start justify-between">
          <div>
            <p className="font-heading text-h3 text-deep-green">{tr("Influențele zilei")}</p>
            {data.context && (
              <p className="font-body text-label-xs text-secondary-text mt-1">
                {tr(data.context.moon)} · {tr(data.context.dayRuler)} · ☀ {tr("în")} {tr(data.context.sunSign)}
              </p>
            )}
            {data.natal && (
              <p className="font-body text-label-xs text-forest-green mt-0.5">
                {tr("Profilul tău:")} ☀ {tr(data.natal.sun)} · Asc {tr(data.natal.ascendant)} · ☾ {tr(data.natal.moon)}
              </p>
            )}
          </div>
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-body font-semibold text-body-sm text-deep-green">{tr(areaLabel)}</span>
                </div>
                <div className="flex items-center gap-2">
...
function DailyInfluenceSkeleton() {
  ...
  return (
    <div className="rounded-2xl border border-sage-border/40 bg-white overflow-hidden shadow-card">
      <div className="px-5 pt-5 pb-4 border-b border-sage-border/30">
        <p className="font-heading text-h3 text-deep-green">{tr("Influențele zilei")}</p>
        <p className="font-body text-label-xs text-secondary-text mt-1 animate-pulse">{tr("Se calculează tranzitele...")}</p>
      </div>
      <div className="divide-y divide-sage-border/20">
        {["Corp", "Minte", "Relații", "Energie"].map(a => (
          <div key={a} className="px-5 py-4">
            <div className="flex items-center justify-between mb-2">
              <span className="font-body font-semibold text-body-sm text-deep-green">{tr(a)}</span>
              <div className="flex gap-0.5">
...
export function DailyInfluencePlaceholder() {
  ...
  return (
    <div className="rounded-2xl border border-sage-border/40 bg-white p-5 text-center shadow-card">
      ...
      <p className="font-heading text-h3 text-deep-green mb-1">{tr("Influențele zilei")}</p>
      <p className="font-body text-body-sm text-secondary-text mb-4">
        {tr("Adaugă data nașterii pentru a vedea cum te influențează tranzitele planetare de azi, personalizat pentru semnul tău.")}
      </p>
      <Link
        href="/dashboard/cont"
