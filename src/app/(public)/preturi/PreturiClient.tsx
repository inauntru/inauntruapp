            ...
            <div key={plan.id} className={`relative rounded-card border p-5 sm:p-8 flex flex-col transition-all duration-200 ${plan.isPopular ? "bg-forest-green border-forest-green text-white shadow-button md:scale-105" : "card card-lift bg-white"}`}>
              {plan.isPopular && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                  <span className="bg-rose-powder text-deep-green text-label-xs font-bold px-4 py-1 rounded-full shadow-sm">{tr("CEL MAI POPULAR")}</span>
                </div>
              )}
              <div className="mb-6">
              ...
              <div className="mb-6">
                ...
                {billingAnnual && plan.price > 0 && <p className={`font-body text-label-xs mt-1 ${plan.isPopular ? "text-white/60" : "text-secondary-text"}`}>= {plan.priceAnnual * 12} {tr("RON/an")} (vs {plan.price * 12} {tr("RON lunar")})</p>}
              </div>
              <ul className="space-y-2.5 mb-8 flex-1">
                {plan.features.map((feature) => (
                  <li key={feature} className="flex items-start gap-2">
                    ...
                    <span className={`font-body text-body-sm ${plan.isPopular ? "text-white/85" : "text-secondary-text"}`}>{tr(feature)}</span>
                  </li>
                ))}
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <AnimateIn from="bottom">
            <div className="text-center mb-10">
              <p className="section-label">{tr("Facturare")}</p>
              <h2 className="section-title">{tr("Întrebări despre plăți")}</h2>
            </div>
          </AnimateIn>
