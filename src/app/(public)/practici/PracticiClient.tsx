...
import { canAccess, contentTier, TIER_LABEL } from "@/lib/plan";
  ...
  return (
    <div ref={ref} className="relative">
      <button onClick={() => setOpen((o) => !o)} className={`filter-pill flex items-center gap-1.5 ${isActive ? "active" : ""}`}>
        {tr(value)}<CaretDown size={12} weight="bold" className={`transition-transform duration-200 ${open ? "rotate-180" : ""}`} />
      </button>
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, scale: 0.95, y: -4 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: -4 }} transition={{ duration: 0.15 }} className="absolute top-full mt-2 left-0 z-50 bg-white border border-sage-border rounded-xl shadow-modal overflow-hidden min-w-[140px]">
            {options.map((opt) => (
              <button key={opt} onClick={() => { onChange(opt); setOpen(false); }} className={`w-full flex items-center justify-between gap-3 px-4 py-2.5 text-body-sm font-body transition-colors text-left ${opt === value ? "bg-light-green text-forest-green font-semibold" : "text-on-surface hover:bg-light-green/50"}`}>
                {tr(opt)}{opt === value && <Check size={13} weight="bold" className="text-forest-green flex-shrink-0" />}
              </button>
            ))}
          </motion.div>
        )}
    ...
    <div className="min-h-screen bg-bg-main">
      ...
      <div className="sticky top-16 lg:top-20 z-30 bg-white/95 backdrop-blur-md border-b border-sage-border shadow-nav">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <div className="hidden md:flex items-center gap-4">
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar">
              {CATEGORIES.map((c) => <button key={c} onClick={() => setCategory(c)} className={`filter-pill ${category === c ? "active" : ""}`}>{tr(c)}</button>)}
            </div>
            ...
            {activeFiltersCount > 0 && <button onClick={clearFilters} className="flex items-center gap-1 text-label-sm text-terracotta hover:text-terracotta/80 transition-colors font-body whitespace-nowrap"><X size={14} weight="bold" />{tr("Resetează")} ({activeFiltersCount})</button>}
          </div>
          <div className="md:hidden flex gap-3">
            ...
            <button onClick={() => setFiltersOpen(true)} className={`relative flex items-center gap-2 filter-pill flex-shrink-0 ${activeFiltersCount > 0 ? "active" : ""}`}>
              <SlidersHorizontal size={16} />{tr("Filtre")}
        ...
        <div className="md:hidden flex gap-2 overflow-x-auto no-scrollbar px-4 pb-3">
          {CATEGORIES.map((c) => <button key={c} onClick={() => setCategory(c)} className={`filter-pill flex-shrink-0 ${category === c ? "active" : ""}`}>{tr(c)}</button>)}
        </div>
      </div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="flex items-center justify-between mb-6">
          <p className="font-body text-body-sm text-secondary-text">{filtered.length} {tr("practici găsite")}</p>
        </div>
        <AnimatePresence mode="wait">
          {filtered.length > 0 ? (
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="tag tag-outline">{tr(practice.level)}</span>
                        </div>
                        {(() => {
                          const tier = contentTier(practice);
                          if (tier === "gratuit") return <span className="tag tag-green">{tr("Gratuit")}</span>;
                          const ok = canAccess(plan, tier);
                          return (
                            <span className={`tag border-0 ${ok ? "bg-forest-green/10 text-forest-green" : "bg-secondary-container text-on-secondary-container"}`}>
                              {ok ? `✓ ${TIER_LABEL[tier]}` : TIER_LABEL[tier]}
                            </span>
                          );
                        })()}
                      </div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-24">
              ...
              <button onClick={clearFilters} className="btn btn-ghost">{tr("Resetează filtrele")}</button>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
          ...
          <>
            ...
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 30 }} className="fixed bottom-0 left-0 right-0 z-50 bg-white rounded-t-2xl p-6">
              ...
              <div className="space-y-5">
                ...
                <div><p className="font-body text-label-sm text-secondary-text uppercase tracking-widest mb-3">{tr("Nivel")}</p><div className="flex flex-wrap gap-2">{LEVELS.map((l) => <button key={l} onClick={() => setLevel(l)} className={`filter-pill ${level === l ? "active" : ""}`}>{tr(l)}</button>)}</div></div>
              </div>
              <div className="flex gap-3 mt-6">
                <button onClick={clearFilters} className="btn btn-ghost flex-1">{tr("Resetează")}</button>
                <button onClick={() => setFiltersOpen(false)} className="btn btn-primary flex-1">{tr("Aplică")}</button>
              </div>
            </motion.div>
          </>
        )}
