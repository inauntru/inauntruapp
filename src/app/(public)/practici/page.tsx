    ...
    <div className="min-h-screen bg-bg-main">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <AnimateIn from="bottom">
            <div className="max-w-2xl">
              <p className="font-body text-label-sm text-primary-fixed-dim uppercase tracking-widest mb-3">
                Bibliotecă practici
              </p>
              <h1 className="font-heading text-h1 text-white mb-4">
                70+ practici somatice
              </h1>
              <p className="font-body text-body-lg text-white/60">
      ...
      <div className="sticky top-16 lg:top-20 z-30 bg-white/95 backdrop-blur-md border-b border-sage-border shadow-nav">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            {activeFiltersCount > 0 && (
              <button onClick={clearFilters} className="flex items-center gap-1 text-label-sm text-terracotta hover:text-terracotta/80 transition-colors font-body whitespace-nowrap">
          ...
          <div className="md:hidden flex gap-3">
            ...
            <button
              ...
              className={`relative flex items-center gap-2 filter-pill flex-shrink-0 ${activeFiltersCount > 0 ? "active" : ""}`}
            >
        ...
        <div className="md:hidden flex gap-2 overflow-x-auto no-scrollbar px-4 pb-3">
          {CATEGORIES.map((c) => (
            <button key={c} onClick={() => setCategory(c)} className={`filter-pill flex-shrink-0 ${category === c ? "active" : ""}`}>
              {c}
            </button>
          ))}
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex items-center justify-between mb-6">
          <p className="font-body text-body-sm text-secondary-text">
            {filtered.length} practici găsite
          </p>
        </div>
        ...
        <AnimatePresence mode="wait">
          {filtered.length > 0 ? (
            <motion.div
              key={`${category}-${duration}-${level}-${search}`}
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
                    ...
                    <Link href={`/practici/${practice.id}`} className="block group card card-lift overflow-hidden h-full">
                      ...
                      <div className="relative aspect-video overflow-hidden">
                        ...
                        <div className="absolute top-3 left-3">
                          <span className="tag tag-green shadow-sm">{practice.category}</span>
                        </div>
                        {/* Lock icon for premium */}
                        {practice.isPremium && (
                          <div className="absolute top-3 right-3">
                            <div className="w-7 h-7 bg-deep-green/80 backdrop-blur rounded-full flex items-center justify-center">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                          <div className="w-12 h-12 bg-forest-green rounded-full flex items-center justify-center shadow-button">
            ...
            >
              <div className="w-20 h-20 bg-light-green rounded-full flex items-center justify-center mx-auto mb-4">
                <MagnifyingGlass size={32} weight="regular" className="text-forest-green/50" />
              </div>
              <h3 className="font-heading text-h3 text-deep-green mb-2">Nicio practică găsită</h3>
              <p className="font-body text-body-sm text-secondary-text mb-6">Încearcă să ajustezi filtrele sau căutarea.</p>
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={() => setFiltersOpen(false)} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-light-green">
                  <X size={18} />
                </button>
              </div>
              <div className="space-y-5">
                <div>
                  <p className="font-body text-label-sm text-secondary-text uppercase tracking-widest mb-3">Durată</p>
                  <div className="flex flex-wrap gap-2">
                    {DURATIONS.map((d) => (
                      <button key={d} onClick={() => setDuration(d)} className={`filter-pill ${duration === d ? "active" : ""}`}>{d}</button>
                    ))}
                  </div>
                </div>
                <div>
                  <p className="font-body text-label-sm text-secondary-text uppercase tracking-widest mb-3">Nivel</p>
                  <div className="flex flex-wrap gap-2">
                    {LEVELS.map((l) => (
                      <button key={l} onClick={() => setLevel(l)} className={`filter-pill ${level === l ? "active" : ""}`}>{l}</button>
                    ))}
                  </div>
                </div>
              </div>
